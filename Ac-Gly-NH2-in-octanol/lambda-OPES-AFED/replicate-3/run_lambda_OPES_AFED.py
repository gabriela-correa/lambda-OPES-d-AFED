...
mass_lbda = 1*dalton*(nanometer)**2

tau_phi_s = 40*femtoseconds
T_phi_s = 1000*kelvin
mass_phi_s = 50.0*dalton*(nanometer/radian)**2
kappa_phi = 1000.0*kilojoule_per_mole/radian**2

tau_psi_s = 40*femtoseconds
T_psi_s = 1000*kelvin
mass_psi_s = 50.0*dalton*(nanometer/radian)**2
kappa_psi = 1000.0*kilojoule_per_mole/radian**2
...
res = next(r for r in top.topology.residues() if r.name == res_name_solute)
...
A3 = name2idx["C3"]
A4 = name2idx["C4"]
phi = cvpack.Torsion(A1, A2, A3, A4, name="phi")

B1 = name2idx["N1"]
B2 = name2idx["C3"]
B3 = name2idx["C4"]
B4 = name2idx["N2"]
psi = cvpack.Torsion(B1, B2, B3, B4, name="psi")
...
phi_s = xps.DynamicalVariable(name="phi_s", unit=radian, mass=mass_phi_s, bounds=xps.CircularBounds())

psi_s = xps.DynamicalVariable(name="psi_s", unit=radian, mass=mass_psi_s, bounds=xps.CircularBounds())

coupling = xps.CollectiveVariableCoupling(f"{U}", [UsoftLJ, ULJCoul], [theta_dv]) + xps.HarmonicCoupling(collective_variable=phi, dynamical_variable=phi_s, force_constant=kappa_phi) + xps.HarmonicCoupling(collective_variable=psi, dynamical_variable=psi_s, force_constant=kappa_psi)
...
extension = xps.integrators.MultiBathRegulatedNHLIntegrator(temperature=None, timeConstant=None, stepSize=dt, forceFirst=True, bathTemperatures=[T_phi_s, T_psi_s, T_lbda], bathTimeConstants=[tau_phi_s, tau_psi_s, tau_lbda])
...
opes_conv = xps.ExtendedSpaceOPES(system=ext_system,
                                  ...
                                  temperature=T_lbda,
                                  barrier=210.0*kilojoules_per_mole,   
                                  frequency=400,
                                  biasFactor=85.0,
                                  exploreMode=False
                                  )
...
simulation.context.setDynamicalVariableValues([cv_values["phi"], cv_values["psi"], 1.0])
...
reporter1 = cvpack.reporting.StateDataReporter('output.csv', 
                                               ...
                                               temperature=True, 
                                               writers=[
                                               xps.DvRegulatedTemperaturesWriter(bath_temperatures=[T_phi_s, T_psi_s, T_lbda],kinetic=True),
                                               xps.ExtensionWriter(kinetic=True, temperature=True, 
